...
% ---- NEW: the convergence withdrawal, dated after the frames above were written ----
\begin{frame}{3e. Both Bands Above Are Withdrawn}
\framesubtitle{A finite-size test run after these slides were written, 2026-09-14/15}
...
bound. Tested on four demonstration nets (pcu, tbo, sod, fcu) at six caps on $|q|$
(10, 8, 5, 4, 3, 2): \textbf{none converged} --- the drift between the two largest supercells
tested stayed above our 5\% tolerance at every cap, on every net.
\end{block}
\begin{block}{\footnotesize Both published bands were also measured on the wrong graph}
\scriptsize
Both the 77-framework and 61-framework bands in slides 3c--3d were computed on the
...
above.
\end{block}
\begin{block}{\footnotesize What this means for slides 3c and 3d}
\scriptsize
Neither is a structural or chemical finding. Both are kept, with this notice, for audit ---
the same treatment given to the pore-size and four-structure withdrawals already on this deck.
The +0.62 correlation with graph size in 3c was not a confound contaminating the descriptor:
it \textbf{was} the descriptor. See \texttt{CLAUDE.md} and the project Results page for the
full account, and \S 8 (Outstanding Work) for what would fix it.
\end{block}
\end{frame}
